...
try {
	...
	function server(opts = {}) {
		...
		async function handler(req, res) {
			...
			if (req.headers.upgrade && req.headers.upgrade.toLowerCase() === 'websocket' && req.headers.connection.toLowerCase().includes('upgrade')) {
			...
			server.on('upgrade', (req, socket, head) => {
				if (req.headers.upgrade.toLowerCase() === 'websocket') {
					wss.handleUpgrade(req, socket, head, (ws) => {
						req.wsInstance = ws;
						const obj = find('WS', req.url, routes, handlers);
						if (obj) {
							req.params = obj.params;
							obj.handlers[0](req, socket, ws);
						} else {
							socket.destroy();
						}
					});
		...
		function ws(pattern, ...fns) {
			add('WS', pattern, ...fns);
		}

		function static(basePath, dirPath) {
			if (!fs.existsSync(dirPath)) {
				throw new Error(`Directory ${dirPath} does not exist.`);
			}

			add('GET', basePath + '/*', async (req, res) => {
				const filePath = path.join(dirPath, req.path.replace(basePath, ''));
